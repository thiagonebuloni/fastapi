...
def test_get_all_posts(authorized_client, test_posts):
    ...
    def validate(post: dict):
        return schemas.PostOut(**post)  # type: ignore

    posts_map = map(validate, res.json())  # type: ignore
    posts = list(posts_map)  # type: ignore
    # print(posts)
...
def test_update_other_user_psot(authorized_client, test_user, test_user2, test_posts):
    data = {
        "title": "updated title",
        "content": "updated content",
        "id": test_posts[3].id,
...
def unauthorized_user_update_post(client, test_user, test_posts):
